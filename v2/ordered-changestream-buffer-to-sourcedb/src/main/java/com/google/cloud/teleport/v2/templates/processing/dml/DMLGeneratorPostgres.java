...
public class DMLGeneratorPostgres {
  private static final Logger LOG = LoggerFactory.getLogger(DMLGeneratorPostgres.class);
  ...
  public static String getDMLStatement(
    ...
    if (sourceTable.getPrimaryKeys() == null || sourceTable.getPrimaryKeys().length == 0) {
      LOG.warn(
          ...
          sourceTable.getName()); // TODO need to implement dead lettering
      return "";
    }
  ...
  private static String getUpsertStatement(
    ...
    if (columnNameValues.size() == 0) { // if there are only PKs
      ...
      LOG.warn("Generated upsert query: " + returnVal);
      return returnVal;
    }
  ...
  private static String getDeleteStatement(
    ...
    LOG.warn("Generated delete query: " + returnVal);
    return returnVal;
  }
  ...
  private static String getMappedColumnValue(
    ...
    if ("FLOAT64".equals(colType)) {
      ...
      colInputValue =
        ...
        && ("bit".equals(sourceColDef.getType().getName())
            || "bit varying".equals(sourceColDef.getType().getName()))) {
    ...
    LOG.warn("Spanner column definition value is : " + spannerColDef.toString());
    LOG.warn("Source column definition value is : " + sourceColDef.toString());
    String response =
  ...
  private static String getColumnValueByType(
    ...
    LOG.warn(" The columnType is : " + columnType + " and the colValue is : " + colValue);
    String response = "";
